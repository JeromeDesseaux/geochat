...
<div>
  ...
  <div class="home" v-else>
    <v-card class="my-3" light v-for="(chatroom, index) in chatrooms" :key="index">
      ...
      <v-card-actions>
        <v-btn text v-if="isParticipant(chatroom)" color="brown lighten-2">Entrer dans le salon</v-btn>
...
export default {
  ...
  methods: {
      getDistance: function(chatroom) {
          return 1000*parseInt(distance(this.user.coordinates[1], this.user.coordinates[0], chatroom.location.coordinates[1], chatroom.location.coordinates[0], "K"));
      },
      getParticipants: function(chatroom) {
          if(chatroom.participants){
              return chatroom.participants.filter(part => part.status === "ACCEPTED").length + 1;
          }
          return 1;
      },
      isParticipant: function(chatroom) {
          if(chatroom.admin === this.user.id) {return true;}
