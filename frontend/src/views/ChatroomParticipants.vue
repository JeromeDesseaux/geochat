<template>
  <div>
    ...
    <div v-else>
      ...
      <h2 class="headline my-5">{{ chatroom.participants.length }} participants dont {{pending.length}} en attente</h2>
      <v-row justify-center>
        <v-col>
          <v-btn color="success" class="mr-5" @click="sendRequest(true)">Accepter</v-btn>
          <v-btn color="red lighten-1" dark @click="sendRequest(false)">Refuser</v-btn>
        </v-col>
        <v-col>
          <v-text-field
            v-model="search"
